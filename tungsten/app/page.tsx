...
export default function Home() {
  ...
  const subTagline = ['NO ONE WILL KNOW.'];
  ...
  const timings = [0, 0.5, 1, 1.8, 3, 3.5, 4.0, 4.5];  // extra paus före "NO ONE WILL KNOW." och "We work with..."

  return (
    <main className="relative min-h-screen overflow-hidden font-jetbrains pb-[200px]">
      <video
        ...
        playsInline
      >
        <source src="/background/bg-metal-hero.mp4" type="video/mp4" />
      </video>
      <div className="absolute inset-0 bg-gradient-to-b from-black/90 via-black/40 to-black/90"></div>

      <div className="relative z-10 flex flex-col items-start justify-center h-full px-6 md:px-12 lg:px-24 pt-24">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <div className="space-y-4">
          {[...tagline, ...subTagline, ...paragraphs].map((line, i) => (
            <motion.p
              key={i}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 1 + timings[i], duration: 0.6 }}
              className={`text-white ${i < tagline.length + subTagline.length ? 'text-[32px] leading-[36px] uppercase' : 'text-[16px] leading-[22px]'} mt-4`}
            >
              {line}
            </motion.p>
          ))}
